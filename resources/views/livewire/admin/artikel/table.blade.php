<div>
    ...
    <table class="table table-striped table-bordered align-middle">
        ...
        <tbody>
            @forelse($post as $d)
            <tr wire:key="artikel-{{ md5($d->id_post) }}">
                ...
                <td class="text-center">
                    <a href="{{ route('artikel.edit', $d->id_post) }}" title="Lihat Detail / Edit" class="btn mb-1 bg-info-subtle text-info rounded-circle round-40 btn-sm d-inline-flex align-items-center justify-content-center me-1">
                    ...
                    @if (auth()->user()->hasRole('Superadmin'))
                        @if ($d->status == 1)
                            <button wire:click="accPost({{ $d->id_post }})" title="ACC & Publish Artikel" class="btn mb-1 bg-success-subtle text-success rounded-circle round-40 btn-sm d-inline-flex align-items-center justify-content-center border-0 me-1">
                        ...
                        @elseif ($d->status == 2)
                            <button wire:click="cancelPost({{ $d->id_post }})" title="Cancel / Unpublish Artikel" class="btn mb-1 bg-secondary-subtle text-secondary rounded-circle round-40 btn-sm d-inline-flex align-items-center justify-content-center border-0 me-1">
                    ...
                    @if (auth()->id() == $d->id_user || auth()->user()->hasRole('Superadmin'))
                        @if ($d->status < 2 || auth()->user()->hasRole('Superadmin'))
                            <a href="{{ route('artikel.edit',$d->id_post) }}" title="Edit" class="btn mb-1 bg-primary-subtle rounded-circle round-40 btn-sm d-inline-flex align-items-center justify-content-center me-1">
